...
int main(int argc, char* argv[])
{
	...
	int cm_dr = atoi(argv[3]);
	int cm_cr = atoi(argv[4]);
	const int n = cm_n + 1;
	...
	while (r_counter != n - 1)
	{
		...
		for (int i = 1; i < n; i++)
			for (int j = 0; j < n; j++)
				if (rflag[i] != 1 && rflag[j] == 1)
					if (distance(x[i], x[j], y[i], y[j]) < min_distance)
					{
	...
	for (int i = 1; i < n; i++)
	{
		...
		if (noh[i]>dr)
		{
			...
			while (tn_01 != 0)
			{
				if (distance(x[tn_01], x[next_node[tn_01]], y[tn_01], y[next_node[tn_01]]) < min_distance)
				{
